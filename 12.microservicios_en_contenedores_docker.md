...
custom:
  auth-server:
    issuer: ${AUTH_SERVER_ISSUER:http://127.0.0.1:9000}
````

Si observamos los cambios anteriores, podemos ver que también hemos agregado una configuración adicional
`custom.auth-server.issuer` cuyo valor está determinado por la variable de entorno `AUTH_SERVER_ISSUER`. Si esta
variable de entorno no está definida, entonces por defecto tomará el valor `http://127.0.0.1:9000`.

La configuración `custom.auth-server.issuer` lo inyectaremos en el método `authorizationServerSettings()` de la clase
`SecurityConfig` de nuestro `authorization-server`. Recordemos que este método configura el servidor de autorización
para identificarse con la `URI` definida en la variable de entorno `AUTH_SERVER_ISSUER` o si no viene, por defecto
tomará como valor la uri `http://127.0.0.1:9000`. Esto es fundamental para validar correctamente los
`tokens JWT emitidos` por este servidor.
...
En la configuración anterior vemos que he agregado a los `issuer-uri` el valor
`${AUTH_SERVER_URL:http://127.0.0.1:9000}`, donde está definido una variable de entorno llamada `AUTH_SERVER_URL`, de
tal manera que cuando creemos el contenedor debemos asignar un valor a esta variable. En caso no lo hagamos, tomará
por defecto el valor `http://127.0.0.1:9000`.

Si no definimos la variable de entorno `AUTH_SERVER_URL` tendrá como único valor `http://127.0.0.1:9000`, pero como
estamos trabajando con contenedores `Docker`, va a fallar dado que cuando levantemos el contenedor de `gateway` buscará
la dirección del servidor de autorización `http://127.0.0.1:9000/.well-known/openid-configuration` y no lo encontrará
porque la dirección `127.0.0.1` hace referencia al propio contenedor gateway, mientras que el authorization server está
en otro contenedor.

Es importante recalcar que el valor `AUTH_SERVER_URL` debe ser el mismo que definamos en el servidor de autorización,
precisamente el mismo valor que le definamos a la variable `AUTH_SERVER_ISSUER` cuando creemos el contenedor de dicho
...
En el siguiente comando usaremos la variable de entorno `AUTH_SERVER_ISSUER` que definimos en el `application.yml` del
`authorization-server` donde definiremos la dirección `IP` real de nuestra pc local.

De esta manera, el `authorization-server` emitirá los `JWT` con el atributo `iss` conteniendo el valor que le estamos
definiendo. **Este valor debe coincidir con el que esperan los recursos protegidos.**

Procedemos a crear el contendor usando la variable de entorno `AUTH_SERVER_ISSUER` cuyo valor representa la dirección
`IP` de mi máquina local.

````bash
$ docker container run -d -p 9000:9000 --name c-authorization-server -e AUTH_SERVER_ISSUER=http://192.168.1.21:9000 --network microservices-project-net authorization-server
da3fa49b6b683b7f91ceba2554417e00cbdc05dc9e44eb0a1174f0517cd25a71
````
...
$ docker container ls -a
CONTAINER ID   IMAGE                     COMMAND                  CREATED              STATUS                       PORTS                               NAMES
da3fa49b6b68   authorization-server      "/__cacert_entrypoin…"   About a minute ago   Up About a minute            0.0.0.0:9000->9000/tcp              c-authorization-server
8468b4747cb5   user-service              "/__cacert_entrypoin…"   4 hours ago          Up About an hour                                                 c-user-service
71fd6b928ea2   item-service              "/__cacert_entrypoin…"   5 hours ago          Up About an hour             0.0.0.0:8085->8085/tcp              c-item-service
761e8e5d7932   product-service           "/__cacert_entrypoin…"   22 hours ago         Up About an hour                                                 c-product-service
dd4f55858258   openzipkin/zipkin:3.5.1   "start-zipkin"           25 hours ago         Up About an hour (healthy)   9410/tcp, 0.0.0.0:9411->9411/tcp    c-zipkin
07f5e85a4681   mysql:8.0.41-debian       "docker-entrypoint.s…"   28 hours ago         Up About an hour             33060/tcp, 0.0.0.0:3308->3306/tcp   c-mysql-zipkin
d69f3ce25cfd   postgres:17-alpine        "docker-entrypoint.s…"   29 hours ago         Up About an hour             0.0.0.0:5433->5432/tcp              c-postgres
a60e121908c8   mysql:8.0.41-debian       "docker-entrypoint.s…"   30 hours ago         Up About an hour             33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
efdda60f4dbb   discovery-server          "/__cacert_entrypoin…"   3 days ago           Up About an hour             0.0.0.0:8761->8761/tcp              c-discovery-server
683472a028ac   config-server             "/__cacert_entrypoin…"   3 days ago           Up About an hour             0.0.0.0:8888->8888/tcp              c-config-server
````
...
Creamos el contenedor `c-gateway-server` teniendo en cuenta la variable de entorno `AUTH_SERVER_URL` que definimos en el
`application.yml` como un `issuer-uri`. El valor representa la dirección `IP` de mi máquina local. Este valor debe ser
el mismo que usamos en la variable de entorno `AUTH_SERVER_ISSUER` cuando creamos el contenedor del servidor de
autorización.

````bash
$ docker container run -d -p 8090:8090 --name c-gateway-server -e AUTH_SERVER_URL=http://192.168.1.21:9000 --network microservices-project-net gateway-server
f194af19a5409decdd1dfff9fc50d4c33b3f8b94a5e57b751975731fa0c0a667
````
...
$ docker container ls -a
CONTAINER ID   IMAGE                     COMMAND                  CREATED          STATUS                       PORTS                               NAMES
f194af19a540   gateway-server            "/__cacert_entrypoin…"   59 seconds ago   Up 58 seconds                0.0.0.0:8090->8090/tcp              c-gateway-server
da3fa49b6b68   authorization-server      "/__cacert_entrypoin…"   4 minutes ago    Up 4 minutes                 0.0.0.0:9000->9000/tcp              c-authorization-server
8468b4747cb5   user-service              "/__cacert_entrypoin…"   5 hours ago      Up About an hour                                                 c-user-service
71fd6b928ea2   item-service              "/__cacert_entrypoin…"   5 hours ago      Up About an hour             0.0.0.0:8085->8085/tcp              c-item-service
761e8e5d7932   product-service           "/__cacert_entrypoin…"   22 hours ago     Up About an hour                                                 c-product-service
dd4f55858258   openzipkin/zipkin:3.5.1   "start-zipkin"           25 hours ago     Up About an hour (healthy)   9410/tcp, 0.0.0.0:9411->9411/tcp    c-zipkin
07f5e85a4681   mysql:8.0.41-debian       "docker-entrypoint.s…"   28 hours ago     Up About an hour             33060/tcp, 0.0.0.0:3308->3306/tcp   c-mysql-zipkin
...
Procedemos a generar un código de autorización. Aquí estamos usando como un `Authorize URI` el
`http://127.0.0.1:9000/oauth2/authorize` que apunta al contenedor `c-authorization-server`. Es decir, la petición lo
estamos haciendo desde nuestra máquina local hacia el contenedor del servidor de autorización.
...
````bash
$ curl -v -X POST -u gateway-app:123456 -d "grant_type=authorization_code&redirect_uri=https://oauthdebugger.com/debug&code=JfrDY6Ia9IVMNnO66d07_e7V6uqEDd26lTebJp6vKGj8IeaE1if6Zag2nCXZEz0Ekie0sFPpZeY86r0PZCnwnBnmNZIiqqc-o-E7mHsagB3aU7FGAJxD7cyRaR6MXyGO" http://localhost:9000/oauth2/token | jq
>
< HTTP/1.1 200
...
< Transfer-Encoding: chunked
< Date: Tue, 29 Apr 2025 21:42:57 GMT
<
{
  "access_token": "eyJraWQiOiJhZTY5N2IzNC1hNjBjLTQ0MGUtOTRkYi1jNTlkYWVjOGMyZDQiLCJhbGciOiJSUzI1NiJ9..W94XyETEKy7Ts1ua90hWm3fXlwJECpGvzX8coGnbeJRkkjF4t__i8AfMPCrN_y0XJLZI5cXJFVgNrEQlI2zVL8eYgzW_ESKzwlSkVA2B-FEkBUQ2wgHp6Ofw8R0g5bT3ICDZth52K97EKXI7rxN_BWaHBIaoco70ZPDFb1IQQmn_GG6FrDduTZTB_z_3Sw6aUWfykEsEJGfvHGeI6pR3v6LnYlsOFOzlvx3IIbgFMFi4i90VmGf9-XgxJaUXwX7e_CLZLjTxFHgObn1mNMsF6Ivo0agcM3EPNSmB82g3nuS255JpwyjF-MwaYUBn76Upk1VUwZgEmSU_0Ez-7rz-Dw",
  "refresh_token": "P2zDRtRQe_dQximV-cS7nXEGcZ2WZSWweDWVSBcdZTPvcil_ajC0K-d1HpYlk7_Jye9OsmFJTt67feRLmERo-Os_ALpI_C3r9_y6GMYPlxqOnw7j8UCjJ-f8Z3ZFXyEU",
  "scope": "openid profile",
  "id_token": "eyJraWQiOiJhZTY5N2IzNC1hNjBjLTQ0MGUtOTRkYi1jNTlkYWVjOGMyZDQiLCJhbGciOiJSUzI1NiJ9..buHlMY2l73PsNAKVHOYhNNcbYnROZq0EtPbW4t2j96IQYm7OeJCgCuljKXajYz2_sgdkgUPdX4p7sec99oUqz4GvXUn0MaPZeQd9h68Htlo9HsPSd9g-7DpqNbgFK1aAdYsjHQcKxw8Egil1CQoPx0ofe-s2hbZy2TkfG0P11L3XbPYBGfsdbVSfvQ1NtXidAtN6setsNSRtygr0VREQW65942c78hu5utmTWzbvZyV5OPcHoqcse4HuQvBtdvxmFP0AlqwihtZmuib4xg-Bru18uEWzgFmzFZm-SXpwqkIafsF4EdFRAmrZ9XoFVJRI1b6YpmvGOWKHw71PkJN3OQ",
  "token_type": "Bearer",
  "expires_in": 3599
}
````
...
`iss` corresponde al valor registrado en el servidor de autorización `http://192.168.1.21:9000` que fue pasado por la
variable de entorno `AUTH_SERVER_ISSUER`.

![15.png](assets/section-12/15.png)

Realizamos una petición para registrar un usuario en el `user-service` pero a través del contendor `c-gateway-server`.

````bash
$ curl -v -X POST -H "Authorization: Bearer eyJraWQiOiJhZTY5N2IzNC1hNjBjLTQ0MGUtOTRkYi1jNTlkYWVjOGMyZDQiLCJhbGciOiJSUzI1NiJ9..W94XyETEKy7Ts1ua90hWm3fXlwJECpGvzX8coGnbeJRkkjF4t__i8AfMPCrN_y0XJLZI5cXJFVgNrEQlI2zVL8eYgzW_ESKzwlSkVA2B-FEkBUQ2wgHp6Ofw8R0g5bT3ICDZth52K97EKXI7rxN_BWaHBIaoco70ZPDFb1IQQmn_GG6FrDduTZTB_z_3Sw6aUWfykEsEJGfvHGeI6pR3v6LnYlsOFOzlvx3IIbgFMFi4i90VmGf9-XgxJaUXwX7e_CLZLjTxFHgObn1mNMsF6Ivo0agcM3EPNSmB82g3nuS255JpwyjF-MwaYUBn76Upk1VUwZgEmSU_0Ez-7rz-Dw" -H "Content-type: application/json" -d "{\"username\": \"dev\", \"password\": \"123456\", \"email\": \"dev@gmail.com\"}" http://localhost:8090/api/v1/users | jq
>
< HTTP/1.1 201 Created
< transfer-encoding: chunked
< Location: /api/v1/users/4
< Content-Type: application/json
< Date: Tue, 29 Apr 2025 21:53:48 GMT
...
<
{
  "id": 4,
  "username": "dev",
  "password": "$2a$10$YyIC6LeX3Oz66t0m51K6hOGkMiDc93BaXV.AAH4R2/qWIuC/w568a",
  "enabled": true,
  "email": "dev@gmail.com",
  "roles": [
    {
