...
## Configuración del Endpoint /actuator/health en Spring Boot (item-service)

Por defecto, al agregar la dependencia `spring-boot-starter-actuator` en el `pom.xml` de un servicio `Spring Boot`, el
endpoint `/actuator/health` se habilita automáticamente sin necesidad de configuraciones adicionales. Sin embargo, si
decides exponer otros endpoints, como el `/actuator/refresh`, a través de la configuración en el archivo
`application.yml`, es necesario asegurarse de incluir explícitamente el endpoint `health` en la sección
`management.endpoints.web.exposure.include`.

### Caso 1: Solo agregar la dependencia `spring-boot-starter-actuator`

Cuando solo agregas la dependencia de `Actuator` en el `pom.xml`, el endpoint `/actuator/health` estará disponible
automáticamente sin necesidad de configuraciones adicionales.

### Caso 2: Agregar endpoints adicionales como `/actuator/refresh`

Si agregas otros endpoints a través de la propiedad `management.endpoints.web.exposure.include` en el archivo
`application.yml` (por ejemplo, para habilitar el endpoint `/actuator/refresh`), es necesario incluir explícitamente el
endpoint `health` en dicha lista para que también quede habilitado.

En el archivo de configuración del `application.yml` de nuestro microservicio `item-service` agregamos la siguiente
configuración para exponer ambos endpoints `/actuator/health` y `/actuator/refresh`.
...
`Nota`: Si no se agrega explícitamente `health` a la lista de endpoints expuestos, el endpoint `/actuator/health` no
estará disponible, incluso si la dependencia `spring-boot-starter-actuator` está presente.

## Despliegue de servicio User, OAuth2 y Gateway Server con docker compose

En este apartado realizamos la configuración para los microservicios restantes de nuestro proyecto.
...
Luego, si ejecutamos docker compose, veremos que la ejecución ha sido exitosa.
...
[+] Running 11/11                                                                                                         
 ...
 ✔ Container c-mysql-zipkin          Healthy                                                                              
 ✔ Container c-zipkin                Healthy                                                                              
 ✔ Container c-user-service          Running                                                                              
 ✔ Container c-product-service       Running                                                                              
 ✔ Container c-authorization-server  Healthy                                                                              
 ✔ Container c-item-service          Healthy                                                                              
 ✔ Container c-gateway-server        Running                                                                              
````

`Importante: `Si hemos realizado algún cambio en el código fuente, es importante agregar la bandera `--build` al comando
anterior, es decir: `docker compose up -d --build`, para que docker vuelva a construir la imagen con el código
actualizado.

Si listamos los contenedores veremos que todos están funcionando correctamente.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE                     COMMAND                  CREATED          STATUS                    PORTS                               NAMES
17e3ffedea71   gateway-server            "/__cacert_entrypoin…"   26 minutes ago   Up 25 minutes             0.0.0.0:8090->8090/tcp              c-gateway-server
04d77d8d4191   item-service              "/__cacert_entrypoin…"   26 minutes ago   Up 26 minutes (healthy)   0.0.0.0:8085->8085/tcp              c-item-service
6e499c8955d4   authorization-server      "/__cacert_entrypoin…"   26 minutes ago   Up 26 minutes (healthy)   0.0.0.0:9000->9000/tcp              c-authorization-server
e1a13eb92e03   product-service           "/__cacert_entrypoin…"   26 minutes ago   Up 26 minutes                                                 c-product-service
b5c4f15235e8   user-service              "/__cacert_entrypoin…"   26 minutes ago   Up 26 minutes                                                 c-user-service
613df86fef31   discovery-server          "/__cacert_entrypoin…"   26 minutes ago   Up 26 minutes (healthy)   0.0.0.0:8761->8761/tcp              c-discovery-server
bbbf14427dd6   config-server             "/__cacert_entrypoin…"   26 minutes ago   Up 26 minutes (healthy)   0.0.0.0:8888->8888/tcp              c-config-server
e32ed7f3153f   openzipkin/zipkin:3.5.1   "start-zipkin"           2 hours ago      Up 2 hours (healthy)      9410/tcp, 0.0.0.0:9411->9411/tcp    c-zipkin
a921eb5de50c   postgres:17-alpine        "docker-entrypoint.s…"   19 hours ago     Up 3 hours (healthy)      0.0.0.0:5433->5432/tcp              c-postgres
d0b8174eec7f   mysql:8.0.41-debian       "docker-entrypoint.s…"   19 hours ago     Up 3 hours (healthy)      33060/tcp, 0.0.0.0:3308->3306/tcp   c-mysql-zipkin
0974babfc235   mysql:8.0.41-debian       "docker-entrypoint.s…"   19 hours ago     Up 3 hours (healthy)      33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
````

Realizamos peticiones para ver su funcionamiento.
...
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Fri, 02 May 2025 18:17:13 GMT
< token-response: 123456
< Cache-Control: no-cache, no-store, max-age=0, must-revalidate
< Pragma: no-cache
< Expires: 0
...
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Fri, 02 May 2025 18:18:16 GMT
< token-response: 123456
...
Generamos un código de autorización logueándonos con el usuario `admin`. Este proceso lo hacemos apoyándonos de la
página `https://oauthdebugger.com/` tal como lo hemos venido haciendo en capítulos anteriores.

![17.png](assets/section-12/17.png)

Con el código de autorización obtenido, podemos realizar la siguiente petición para obtener un `access_token`.

````bash
$ curl -v -X POST -u gateway-app:123456 -d "grant_type=authorization_code&redirect_uri=https://oauthdebugger.com/debug&code=jr_QEFegpnZDnqvYkIowQG_hkrn7dEAAZdo833JN6GBi3JUpqdou9n8wB9zj3NZBtSaqZ5roAlWZzYhSv6v44uLzH1N1fIpbspZeV44nfavfwrYC8eZExqqm0YjhRBME" http://localhost:9000/oauth2/token | jq
>
< HTTP/1.1 200
< X-Content-Type-Options: nosniff
< X-XSS-Protection: 0
...
< Content-Type: application/json;charset=UTF-8
< Transfer-Encoding: chunked
< Date: Fri, 02 May 2025 20:24:36 GMT
<
{
  "access_token": "eyJraWQiOiJkMTU0OWJiMy0wZTc0LTQ3MjYtYmUwNS00ZjcxZjlmZDg5ZGYiLCJhbGciOiJSUzI1NiJ9..kVZV_aaCJO57hiIChlMvWjIOjzxgAaSwFzeJ7NNxHpPY8Hfv9HyBGYnBqEeQeCBytgYiq4rg46eutLR8BCBU41XKNu_AhsUWTGHSy-mrthTHEJyrMK9EsBkekrapFOtuQL9eSUfxUGJgaCEPoWGhY4YHqpCzPvlQ_Qr2prgNXtIDf9GsywilA_2fsQxWF3i_idArAGTBTv3XCZ45l36mconTRZ4mkAWRWIw7Y2OgyR_2UshjtlNnWDGcZLSVWtBKYGREdCnRKx0GI0t1QJrUGrLZnhyS2b0fgYjXtIwods3dSzFEppbPjZ1iYlavgCfos3nddfBP19Y9zdUpUuCaSA",
  "refresh_token": "y3W33gIcbneD4WNBAhdcdY6ENHNO71TvpfYp1TKjkb5XbCWR5yxTjme2YFBHTMj9aLwY_37fe99J6XVdvZEUR_mbuQwcDbRYMPWhmsTjzXBnT6PX9gzKBWgT4f1hvyKj",
  "scope": "openid profile",
  "id_token": "eyJraWQiOiJkMTU0OWJiMy0wZTc0LTQ3MjYtYmUwNS00ZjcxZjlmZDg5ZGYiLCJhbGciOiJSUzI1NiJ9..bvLXP72NedEUoFfqL9hnC4pfXRVstlPRmqAOtBMd_xtPFyvuae0h7KUbaRxnJa2xRrT2DNvKtX2u6PAlpz5hFiomOZjhTtY8amIezWXdhdaiHzVymwNXdly2R2H1LBAEHcmjdlaeY3KnJMzBQe0CVAYgM_Vd1OUF2nKz6wBz4Nfc9zfHhDXSjvlUEMkQ4z2Hc9mGR2sqaKgjZHMzt-VwOAvjleNbScXraI56U6kkxBileWattMe0Gqu8e5HEkBoPH97Ym3FxDTQz8kElzKgs0h7slKVJR8lBx8AU68nuckdygB7DAEjPW3TAFF-qVFnSm8E34Ab0CX9bRaAcAgWzww",
  "token_type": "Bearer",
  "expires_in": 3599
}
````

Una vez obtenido el access token, lo probamos para ver si está funcionando.

````bash
$ curl -v -X POST -H "Authorization: Bearer eyJraWQiOiJkMTU0OWJiMy0wZTc0LTQ3MjYtYmUwNS00ZjcxZjlmZDg5ZGYiLCJhbGciOiJSUzI1NiJ9..kVZV_aaCJO57hiIChlMvWjIOjzxgAaSwFzeJ7NNxHpPY8Hfv9HyBGYnBqEeQeCBytgYiq4rg46eutLR8BCBU41XKNu_AhsUWTGHSy-mrthTHEJyrMK9EsBkekrapFOtuQL9eSUfxUGJgaCEPoWGhY4YHqpCzPvlQ_Qr2prgNXtIDf9GsywilA_2fsQxWF3i_idArAGTBTv3XCZ45l36mconTRZ4mkAWRWIw7Y2OgyR_2UshjtlNnWDGcZLSVWtBKYGREdCnRKx0GI0t1QJrUGrLZnhyS2b0fgYjXtIwods3dSzFEppbPjZ1iYlavgCfos3nddfBP19Y9zdUpUuCaSA" -H "Content-type: application/json" -d "{\"username\": \"prod\", \"password\": \"123456\", \"email\": \"prod@gmail.com\"}" http://localhost:8090/api/v1/users | jq
>
< HTTP/1.1 201 Created
< transfer-encoding: chunked
< Location: /api/v1/users/3
< Content-Type: application/json
< Date: Fri, 02 May 2025 20:27:16 GMT
< token-response: 123456
< Cache-Control: no-cache, no-store, max-age=0, must-revalidate
< Pragma: no-cache
< Expires: 0
...
<
{
  "id": 3,
  "username": "prod",
  "password": "$2a$10$Ca1OY9gequTP7zoVQcWqn.VrWTncmT7XpRuCIDVqaa5rxDLY2UL.C",
  "enabled": true,
...
## Eliminación del atributo `container_name` para permitir escalabilidad

Eliminaré el atributo `container_name` de los servicios que utilizan puertos aleatorios, en este caso
`s-product-service` y `s-user-service`. Esto se debe a que dichos servicios están diseñados para ser escalados
(es decir, para ejecutarse con múltiples instancias).

Cuando definimos un `container_name`, `Docker` impone un nombre fijo al contenedor, lo cual entra en conflicto al
intentar crear múltiples instancias del mismo servicio, ya que el nombre se volvería duplicado. Por esta razón,
al escalar servicios, se recomienda no establecer el atributo `container_name`, permitiendo así que `Docker` genere
automáticamente un nombre único para cada instancia.

## Escalando más instancias con `--scale` de docker compose

Para escalar un servicio específico (`s-product-service`) y ejecutar múltiples instancias del mismo, se utiliza el
siguiente comando:

````bash
D:\programming\spring\01.udemy\02.andres_guzman\06.microservices_netflix_eureka\microservices-project (feature/section-12)
$ docker compose up -d --scale s-product-service=3                                                                        
[+] Running 13/13                                                                                                         
 ✔ Container c-discovery-server                         Healthy                                                           
 ✔ Container c-mysql                                    Healthy                                                           
 ✔ Container c-postgres                                 Healthy                                                           
 ...
 ✔ Container c-mysql-zipkin                             Healthy                                                           
 ✔ Container c-zipkin                                   Healthy                                                           
 ✔ Container microservices-project-s-user-service-1     Running                                                           
 ✔ Container microservices-project-s-product-service-1  Running                                                           
 ✔ Container microservices-project-s-product-service-3  Started                                                           
 ✔ Container microservices-project-s-product-service-2  Started                                                           
 ✔ Container c-authorization-server                     Healthy                                                           
 ✔ Container c-item-service                             Healthy                                                           
 ✔ Container c-gateway-server                           Running                                                           
````

Este comando le indica a `Docker Compose` que inicie la aplicación en segundo plano (`-d`) y que `escale` el servicio
`s-product-service` a `3 instancias`. Es importante destacar lo siguiente:

- Cada instancia del servicio se ejecutará en un contenedor separado.


- `Docker` asignará automáticamente un nombre único a cada contenedor del servicio (por ejemplo,
  `microservices-project-s-product-service-1`, `microservices-project-s-product-service-2`,
  `microservices-project-s-product-service-3`), por lo que no se debe definir el atributo `container_name` en el
  archivo `compose.yml` para este servicio.


- El uso de `puertos aleatorios` (o el uso de redes internas para la comunicación entre servicios) es recomendable,
  ya que exponer múltiples instancias en un mismo puerto no es posible.

Este enfoque permite probar comportamientos de `balanceo de carga`, `descubrimiento de servicios`, o simplemente
simular una arquitectura más cercana a producción.

Si listamos todos los contenedores, vemos que las 3 instancias del `product-service` se han creado correctamente.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE                     COMMAND                  CREATED          STATUS                    PORTS                               NAMES
a3244f22e4a9   product-service           "/__cacert_entrypoin…"   3 minutes ago    Up 3 minutes                                                  microservices-project-s-product-service-2
e5f89f3ca79f   product-service           "/__cacert_entrypoin…"   3 minutes ago    Up 3 minutes                                                  microservices-project-s-product-service-3
8b67e9e6268c   product-service           "/__cacert_entrypoin…"   11 minutes ago   Up 10 minutes                                                 microservices-project-s-product-service-1
70854ab6031b   user-service              "/__cacert_entrypoin…"   16 minutes ago   Up 10 minutes                                                 microservices-project-s-user-service-1
17e3ffedea71   gateway-server            "/__cacert_entrypoin…"   3 hours ago      Up 10 minutes             0.0.0.0:8090->8090/tcp              c-gateway-server
04d77d8d4191   item-service              "/__cacert_entrypoin…"   3 hours ago      Up 10 minutes (healthy)   0.0.0.0:8085->8085/tcp              c-item-service
6e499c8955d4   authorization-server      "/__cacert_entrypoin…"   3 hours ago      Up 10 minutes (healthy)   0.0.0.0:9000->9000/tcp              c-authorization-server
613df86fef31   discovery-server          "/__cacert_entrypoin…"   3 hours ago      Up 11 minutes (healthy)   0.0.0.0:8761->8761/tcp              c-discovery-server
bbbf14427dd6   config-server             "/__cacert_entrypoin…"   3 hours ago      Up 11 minutes (healthy)   0.0.0.0:8888->8888/tcp              c-config-server
e32ed7f3153f   openzipkin/zipkin:3.5.1   "start-zipkin"           5 hours ago      Up 11 minutes (healthy)   9410/tcp, 0.0.0.0:9411->9411/tcp    c-zipkin
a921eb5de50c   postgres:17-alpine        "docker-entrypoint.s…"   22 hours ago     Up 11 minutes (healthy)   0.0.0.0:5433->5432/tcp              c-postgres
d0b8174eec7f   mysql:8.0.41-debian       "docker-entrypoint.s…"   22 hours ago     Up 11 minutes (healthy)   33060/tcp, 0.0.0.0:3308->3306/tcp   c-mysql-zipkin
0974babfc235   mysql:8.0.41-debian       "docker-entrypoint.s…"   22 hours ago     Up 11 minutes (healthy)   33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
````

Si vamos a eureka server, podemos ver que todas nuestras instancias se han registrado.

![18.png](assets/section-12/18.png)

Realizamos peticiones para ver si se está aplicando balanceo de carga. El resultado lo podemos observar en los
distintos puertos que nos retorna en cada petición indicando que sí está aplicando balanceo de carga.

````bash
$ curl -v -H "Authorization: Bearer eyJraWQiOiIzYTJkNzkzOC01ZTZhLTRhZGItYmFmZS0xYTlmNWQwZTc4MDIiLCJhbGciOiJSUzI1NiJ9..Raz5woLGD60KwECmFopE2xRDncJuukPFlaV1l6jafDucVja90gvDSSfQRSt6vA9sfXcQl_KGRtArVDELO26wJxdUpVM4pmfbC2aISek6MnvXUH-ygMymMj1oe_ohWqjcLX9G_UlUIjFKFgszDv3RG-lcpibmrfrcrBNHzXWYk_oAbiAO4gdNTqR-nwFiUKLFgXGdPZJzJk1rFbRafdStM-o315V3EU-LN7wo4mAHNPACo9cjs2NsaVy5xtW8QQDTe0AY1XOlPVgjX-KO5upO95lws_PVUvijDE-TSiyFkOndDN13dwYmg3a48qdmRZXVkiNVcyfUJph1tv_R4D1Y9A" -G --data "quantity=10" http://localhost:8090/api/v1/items/1 | jq
>
< HTTP/1.1 200 OK
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Fri, 02 May 2025 21:10:51 GMT
< token-response: 123456
< X-Response-color: green
< Cache-Control: no-cache, no-store, max-age=0, must-revalidate
< Pragma: no-cache
...
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Fri, 02 May 2025 21:10:48 GMT
< token-response: 123456
< X-Response-color: green
< Cache-Control: no-cache, no-store, max-age=0, must-revalidate
< Pragma: no-cache
...
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Fri, 02 May 2025 21:10:43 GMT
< token-response: 123456
< X-Response-color: green
< Cache-Control: no-cache, no-store, max-age=0, must-revalidate
< Pragma: no-cache
...
## Escalando más instancias con `deploy.replicas` de docker compose

A partir de `Docker Compose V2` (la versión basada en `CLI plugin`, no el binario independiente), la opción
`deploy.replicas` sí es interpretada al usar `docker compose up`, pero solo en ciertos contextos, como:

- Si no estás usando ninguna configuración exclusiva de Swarm (como redes globales o restricciones de placement).

- Si estás usando `Docker Desktop` o `Docker` en una máquina local (no Swarm), `Compose` simplemente crea múltiples
  contenedores con sufijos numerados (`-1`, `-2`, `-3`).

Así que, otra forma sencilla de poder escalar instancias de servicios es utilizando la configuración misma en el
archivo `compose.yml`, tal como se ve a continuación (`deploy.replicas: 3`).
...
Al definir `deploy.replicas: 3` dentro del servicio `s-product-service`, `Docker Compose` crea automáticamente 3
instancias del servicio al ejecutar `docker compose up -d`. Aunque la clave `deploy` está asociada a `Docker Swarm`,
en versiones modernas de `Docker Compose (v2+)`, la propiedad `replicas` se respeta parcialmente y permite escalar
servicios sin necesidad de usar el flag `--scale`.

Entonces levantamos nuestros servicios con el típico comando `docker compose up -d`.
...
[+] Running 13/13                                                                                                          
 ...
 ✔ Container microservices-project-s-product-service-3  Started                                                            
 ✔ Container microservices-project-s-user-service-1     Running                                                            
 ✔ Container microservices-project-s-product-service-1  Started                                                            
 ✔ Container microservices-project-s-product-service-2  Started                                                            
 ✔ Container c-authorization-server                     Healthy                                                            
 ✔ Container c-item-service                             Healthy                                                            
 ✔ Container c-gateway-server                           Running                                                            
...
## Configuración final de servicios en compose.yml

Llegados hasta este punto se muestra a continuación las configuraciones finales con las que se ha terminado este
capítulo. Estas configuraciones están `100%` funcionales, así que las coloco aquí por si en las secciones siguientes
las modificamos.
