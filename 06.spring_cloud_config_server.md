...
En este apartado veremos cómo actualizar los cambios que hagamos en las configuraciones del `repositorio local de git`
sin tener que detener la aplicación.

Para eso, es importante tener la dependencia `spring-boot-starter-actuator` en el microservicio que queremos actualizar
sus propiedades. En nuestro caso, ya tenemos dicha dependencia pues la agregamos al momento de construir el
`item-service`.
...
Esta dependencia nos proporciona una `uri` que nos permite realizar el `refresh` de aquellos componentes
`(@Component, @Controller, @Service, etc.)` que estén anotados con `@RefreshScope`. Para nuestro caso, agregaremos esa
anotación al controlador del `item-service`, dado que en ese componente estamos obteniendo las configuraciones a través
de la anotación `@Value` y la interfaz `Environment`.
...
Ahora, en el `application.yml` del `item-service` vamos a agregar la configuración para activar la uri que nos
proporciona actuator y poder realizar la actualización.

Notar que para esta prueba vamos a trabajar con el perfil `dev`.
...
Cuando usamos `Spring Cloud Config` junto con `@RefreshScope`, podemos actualizar propiedades de configuración en
caliente, sin necesidad de reiniciar la aplicación. Pero para que esto funcione correctamente, el microservicio debe
tener:

1. La anotación `@RefreshScope` en las clases donde se inyectan propiedades que deseas actualizar.
2. El `Spring Boot Actuator` como dependencia `(spring-boot-starter-actuator)`.
3. Expuestos ciertos endpoints del `Actuator`, específicamente el endpoint `/actuator/refresh`.

Por defecto, `Spring Boot Actuator` no expone todos los endpoints públicamente por seguridad, así que necesitas
especificar en tu `application.yml` qué endpoints deseas exponer.

`management.endpoints.web.exposure.include: '*'`, le estamos diciendo a `Spring Boot` que exponga todos los endpoints
del `Actuator` por vía HTTP. Es decir, endpoints como `/actuator/health`, `/actuator/info`, `/actuator/refresh`,
entre otros, estarán disponibles para ser accedidos externamente.

Esto es lo que permite que puedas hacer un `POST` al endpoint `/actuator/refresh` y que la aplicación refresque las
propiedades con `@RefreshScope`.

#### Consideraciones de seguridad

En ambientes productivos no se recomienda usar `include: '*'`, porque estaríamos exponiendo endpoints sensibles. En su
lugar, podemos listar solo los necesarios:
...
Así solo estarán disponibles `/actuator/refresh`, `/actuator/health` y `/actuator/info`.

También podríamos proteger esos endpoints con seguridad (por ejemplo, usando `Spring Security`) o ponerlos detrás de un
gateway seguro.

Levantamos las aplicaciones para hacer pruebas del refresco. Los levantamos en este orden.

- config-server
- discovery-server
- item-service

Como hemos ejecutado la aplicación con el perfil `dev`, vemos que nos retorna la siguiente información.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Mon, 14 Apr 2025 22:53:18 GMT
<
{
  "custom.author.email": "magadiflo@gmail.com",
  "server.port": 8085,
...
Ahora, procedamos a cambiar los valores de dichas propiedades en el `repositorio local de git` y luego los commiteamos.
El archivo que vamos a modificar es el `item-service-dev.yml` y quedaría así.

````yml
custom:
  text: item-service-dev.yml en config server con perfil dev (Actualizado)
  author:
    name: Martín (Actualizado)
    email: magadiflo@gmail.com (Actualizado)
````

Luego, procedemos a guardarlo y commitearlo.

````bash
magadiflo@SysEngJava:/mnt/d/programming/spring/01.udemy/02.andres_guzman/06.microservices_netflix_eureka/config-server-repo$ git add .
magadiflo@SysEngJava:/mnt/d/programming/spring/01.udemy/02.andres_guzman/06.microservices_netflix_eureka/config-server-repo$ git commit -m "Modifica propiedades del item-service-dev.yml"
````

Estamos listos para actualizar las propiedades sin necesidad de detener la aplicación. Para eso realizamos una petición
del tipo `POST` a la siguiente `URL` de nuestro `item-service`.

````bash
$ curl -v -X POST http://localhost:8085/actuator/refresh | jq
>
< HTTP/1.1 200
< Content-Type: application/vnd.spring-boot.actuator.v3+json
< Transfer-Encoding: chunked
< Date: Mon, 14 Apr 2025 23:01:01 GMT
<
[
  "custom.text",
...
Cuando hacemos un `POST` a `http://localhost:8085/actuator/refresh`, `Spring Boot Actuator` con `Spring Cloud Config`
vuelve a cargar las propiedades externas desde el servidor de configuración.
...
Nos está diciendo que estas propiedades fueron actualizadas con éxito gracias al `refresh`. Es decir:

- Se volvieron a cargar desde el servidor de configuración.
- Los beans anotados con `@RefreshScope` que usan estas propiedades han sido recreados internamente con los nuevos
  valores.

Volvemos a hacer la petición al `item-service` y verificamos que las configuraciones se han refrescado correctamente.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Mon, 14 Apr 2025 23:08:47 GMT
<
{
  "custom.author.email": "magadiflo@gmail.com (Actualizado)",
  "server.port": 8085,
  "custom.text": "item-service-dev.yml en config server con perfil dev (Actualizado)",
  "custom.author.name": "Martín (Actualizado)"
}
````

## Consideraciones importantes al usar @RefreshScope y /actuator/refresh

Cuando hacemos un `refresh` con `/actuator/refresh`, no todo en tu aplicación se refresca automáticamente.
Vamos por partes:

1. `Lo que sí se puede refrescar dinámicamente`, con `@RefreshScope`, puedes actualizar en caliente:
    - Propiedades personalizadas (custom.text, app.title, etc.).
    - URLs o flags que usas dentro de tus servicios.
    - Credenciales de servicios externos (si están bien aisladas).
    - Configuraciones de beans propios, si esos beans están anotados con `@RefreshScope`.


2. `Lo que no se refresca automáticamente`, hay ciertos componentes que no se recrean automáticamente con un
   `/actuator/refresh`, incluso si sus propiedades cambian:
    - `Conexiones a base de datos (DataSource)`, cambiar propiedades como `spring.datasource.url`, `username`,
      `password` no reconfigura el pool de conexiones existente. Para que los cambios surtan efecto, deberías reiniciar
      la aplicación o recrear manualmente el DataSource bean (lo cual no es trivial ni recomendable para producción).

    - `Configuración de JPA / Hibernate`, propiedades como `spring.jpa.hibernate.ddl-auto`, `show-sql`, etc., se cargan
      al iniciar la aplicación, y no se refrescan dinámicamente. Un `refresh` no vuelve a escanear entidades ni cambia
      el comportamiento de JPA.

    - `Configuraciones de Spring Security o Bean no anotados con @RefreshScope`, si no usas `@RefreshScope`, el bean no
      se va a actualizar aunque cambies las propiedades.

## Creando repositorio remoto de GitHub y push

Creamos un repositorio remoto en `GitHub` y lo agregamos al repositorio local de git utilizando el comando
`git remote add origin <url_repo_remoto>`.

````bash
$ git remote add origin https://github.com/magadiflo/config-server-repo.git
````

Luego, como ya tenemos vinculado el repo local con el repo remoto, subimos los cambios al repo remoto.

````bash
$ git push -u origin main
````

Si revisamos nuestro repositorio remoto en GitHub veremos que los archivos se han subido correctamente.

![03.png](assets/section-06/03.png)

En el `config-server` necesitamos cambiar la dirección desde donde estamos tomando los archivos de configuraciones,
ahora debe apuntar al repositorio remoto en `GitHub` y ya no al repositorio local como lo veníamos trabajando
anteriormente.

Así que en el `application.yml` del `config-server` realizamos la siguiente modificación.
...
spring:
  ...
  cloud:
    config:
      server:
        git:
          uri: https://github.com/magadiflo/config-server-repo.git
````

Ahora, probaremos los cambios realizados y veremos si se están trayendo las configuraciones del repositorio remoto de
GitHub.
...
- config-server
- eureka-server
- item-service (perfil `dev`)

Como resultado, vemos que la aplicación está leyendo las configuraciones que acabamos de subir a `GitHub`.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Tue, 15 Apr 2025 15:21:57 GMT
<
{
  "custom.author.email": "magadiflo@gmail.com (Actualizado)",
  "custom.author.name": "Martín (Actualizado)",
...
## Actualiza las configuraciones con los cambios subidos al repositorio de GitHub

Realizaremos los siguientes cambios en el archivo `item-service-dev.yml` del repositorio local, lo `commiteamos` y lo
`pusheamos` al repositorio remoto de `GitHub`.

````yml
custom:
  text: item-service-dev.yml en config server con perfil dev (desde GitHub)
  author:
    name: Martín (desde GitHub)
    email: magadiflo@gmail.com (desde GitHub)
````

A continuación, sin detener las aplicaciones vamos a actualizar las configuraciones apoyándonos de actuator y de la uri
que nos proporciona. Como resultado veremos todas las propiedades que han sido refrescadas.
...
< Content-Type: application/vnd.spring-boot.actuator.v3+json
< Transfer-Encoding: chunked
< Date: Tue, 15 Apr 2025 15:27:40 GMT
<
[
  "custom.text",
  "config.client.version",
...
Ahora, si volvemos a realizar la petición para ver los valores de las configuraciones de items veremos los nuevos
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Tue, 15 Apr 2025 15:29:15 GMT
<
{
