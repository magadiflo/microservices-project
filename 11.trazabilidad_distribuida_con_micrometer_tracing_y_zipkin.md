...
## Conectando Zipkin en microservicios

Agregaremos la dependencia de `Zipkin` a los siguientes microservicios:
...
Según el [supported tracer](https://docs.micrometer.io/tracing/reference/tracers.html) utilizado en el proyecto,
utilizaremos su correspondiente [supported reporters](https://docs.micrometer.io/tracing/reference/reporters.html).
En nuestro caso hemos usado la dependencia del `supported tracert` llamado `micrometer-tracing-bridge-brave`,
por lo que su `supported reporter` será el siguiente.
...
Además de la dependencia anterior también agregamos la siguiente configuración en cada uno de sus `application.yml`.
...
Esta configuración define la url donde se enviarán las trazas recopiladas por `Micrometer`. El endpoint mostrado es el
endpoint expuesto por `Zipkin (v2 API)`.

## Observación en la interfaz de Zipkin

Para ver el funcionamiento de `Zipking` ejecutamos las aplicaciones en el siguiente orden.
...
Una vez ejecutados los microservicios y con el servidor de `Zipkin` corriendo en http://127.0.0.1:9411, se puede acceder
a la interfaz web de `Zipkin`. En esta interfaz es posible:

- Ver un listado de los servicios registrados que han enviado trazas recientemente.
- Buscar trazas usando distintos criterios como: nombre del servicio, nombre del endpoint, estado (errores, duración,
  etc.), intervalo de tiempo
- Visualizar cada traza como una línea de tiempo, con los distintos spans (segmentos de una llamada) que muestran el
  recorrido de una petición a través de múltiples microservicios.
- Identificar cuellos de botella, errores y la latencia entre servicios.

![15.png](assets/section-10/15.png)

Por ejemplo, si una solicitud `HTTP` entra por el `API Gateway`, este la redirige al `item-service`, el cual a su vez
realiza una llamada `HTTP` al `product-service`. En la interfaz de `Zipkin` se puede observar la traza completa,
desde que la solicitud es recibida por el `Gateway`, pasando por el `item-service`, hasta que se obtiene la respuesta
del `product-service`. La visualización incluye los tiempos de cada segmento (`span`), permitiendo analizar la
latencia entre servicios y detectar posibles cuellos de botella.

En la siguiente imagen se muestra el servicio `gateway-server` que ha registrado la traza.

![16.png](assets/section-10/16.png)

Si damos clic en el botón `SHOW` vemos que se muestran todos los servicios que han intervenido en el flujo de la
petición.

![17.png](assets/section-10/17.png)

Si ahora realizamos otra petición, por ejemplo al `authorization-server` para solicitar un código de autorización. Este
microservicio se comunicará con nuestro `user-service` para verificar si las credenciales del usuario son válidos o no.

![18.png](assets/section-10/18.png)

Como vemos en la siguiente imagen, el `authorization-server` se está comunicando con el `user-service`.

![19.png](assets/section-10/19.png)

## [Configurando MySQL Storage en Zipkin Server](https://github.com/openzipkin/zipkin/?tab=readme-ov-file#mysql)

Por defecto el componente `InMemoryStorage` está empaquetado en la biblioteca principal de `Zipkin`. No es persistente
ni viable para cargas de trabajo realistas. Su propósito es realizar pruebas; por ejemplo, iniciar un servidor en un
portátil sin necesidad de una base de datos.

El componente `MySQL v1` utiliza las características de `MySQL 5.6+`, pero se ha probado con `MariaDB 10.11`.

El esquema se diseñó para que fuera fácil de entender y comenzar a usar; no para el rendimiento. Por ejemplo, los
campos "spans" son columnas, por lo que se pueden realizar consultas ad-hoc con SQL. Sin embargo, este componente
presenta problemas de rendimiento conocidos: las consultas tardan segundos en devolverse si se introducen muchos datos.

Este almacén no requiere un trabajo para agregar enlaces de dependencia. Sin embargo, ejecutar el trabajo mejorará el
rendimiento de las consultas de dependencia.

Por lo tanto, en este apartado cambiaremos el almacenamiento a `MySQL`, para eso seguimos los siguientes pasos:

1. Creamos una base de datos en MySQL con cualquier nombre, en mi caso lo llamaré `db_zipkin`.
2. Usaremos el script proporcionado por
   [zipkin](https://github.com/openzipkin/zipkin/blob/master/zipkin-storage/mysql-v1/src/main/resources/mysql.sql)
   y lo ejecutamos en la base de datos recién creada.
    ````sql
    --
    ...
    ````
3. Creamos un usuario personalizado llamado `zipkin` cuyo password también será `zipkin`. Lo asignaremos a la base de
   datos `db_zipkin`. Este usuario tendrá los privilegios que se muestran en la siguiente imagen.

   ![20.png](assets/section-10/20.png)

4. Luego de dar en `save` nos aparecerá una ventana con el siguiente script conteniendo los datos del usuario a crear.
    ````sql
    CREATE USER 'zipkin'@'%' IDENTIFIED BY 'zipkin';
    GRANT INSERT ON db_zipkin.* TO 'zipkin'@'%';
    GRANT UPDATE ON db_zipkin.* TO 'zipkin'@'%';
    GRANT SELECT ON db_zipkin.* TO 'zipkin'@'%';
    GRANT DELETE ON db_zipkin.* TO 'zipkin'@'%';
    GRANT SHOW VIEW ON db_zipkin.* TO 'zipkin'@'%';
    GRANT EXECUTE ON db_zipkin.* TO 'zipkin'@'%';
    ````
5. Finalmente, creamos una conexión con nuestro `DBeaver` para la `bd_zipkin` con el usuario `zipkin`.

   ![21.png](assets/section-10/21.png)

## [Levantando Zipkin Server con MySQL](https://github.com/openzipkin/zipkin/tree/master/zipkin-server#mysql-storage)

Para levantar nuestro `zipkin server` con la `bd_zipkin` que creamos anteriormente, debemos seguir la documentación
mostrada en [MySQL Storage](https://github.com/openzipkin/zipkin/tree/master/zipkin-server#mysql-storage). Pero
básicamente lo que debemos hacer es ejecutar el `jar` con el comando `java -jar` y agregarle variables de entorno.

Si estuviéramos trabajando en un entorno `Unix/Linux` las variables de entorno las podríamos colocar directamente
en la línea de comandos seguidos del comando `java -jar`, tal como se ve a continuación.

````bash
$ STORAGE_TYPE=mysql MYSQL_DB=db_zipkin MYSQL_USER=zipkin MYSQL_PASS=zipkin java -jar zipkin-server-3.5.0-exec.jar
````

Pero como estoy trabajando en `Windows`, definiré las variables de entorno y el comando en un archivo `.bat` y
ejecutaré dicho archivo desde la línea de comandos.

Creamos un archivo llamado `start-zipkin.bat` donde agregamos las variables de entorno y el comando para ejecutar el
`jar` de `zipkin`.

````bash
@echo off
set STORAGE_TYPE=mysql
set MYSQL_DB=db_zipkin
...
**Recordemos**
> Un archivo `.bat` (o `batch file`) es un script para `Windows` que contiene una secuencia de comandos que el símbolo
> del sistema (`CMD`) interpreta línea por línea. Se usa para automatizar tareas.
>
> `@echo off`
> - Le dice al `CMD` que no muestre los comandos que se están ejecutando en pantalla.
> - El `@` al inicio evita que esa misma línea (`echo off`) sea mostrada.
> - `Resultado`: solo se muestra la salida del programa, no los comandos mismos.
>
> `set VARIABLE=valor`
> - Define variables de entorno `solo dentro del contexto del script o terminal actual`.
> - En nuestro caso, definimos variables que son leídas por el `zipkin-server-3.5.0-exec.jar` durante su arranque.

Ahora, simplemente a través de la terminal ejecutamos el archivo `start-zipkin.bat`.
...
2025-04-24T16:09:53.148-05:00  WARN [/] 5792 --- [           main] i.m.p.PrometheusMeterRegistry            : A MeterFilter is being configured after a Meter has been registered to this registry. All MeterFilters should be configured before any Meters are registered. If that is not possible or you have a use case where it should be allowed, let the Micrometer maintainers know at https://github.com/micrometer-metrics/micrometer/issues/4920. Enable DEBUG level logging on this logger to see a stack trace of the call configuring this MeterFilter.
2025-04-24T16:09:53.459-05:00  INFO [/] 5792 --- [oss-http-*:9411] c.l.a.s.Server                           : Serving HTTP at /[0:0:0:0:0:0:0:0]:9411 - http://127.0.0.1:9411/
2025-04-24T16:10:22.286-05:00  INFO [/] 5792 --- [cking-tasks-1-1] c.z.h.p.HikariPool                       : HikariPool-1 - Added connection org.mariadb.jdbc.Connection@5aaa7cf5
2025-04-24T16:10:22.995-05:00  INFO [/] 5792 --- [cking-tasks-1-1] o.j.Constants                            :
...
2025-04-24T16:10:23.047-05:00  INFO [/] 5792 --- [cking-tasks-1-1] o.j.i.D.logVersionSupport                : Version                  : Database version is supported by dialect MYSQL: 8.0.41
````

Como vemos, hemos integrado correctamente el `Zipkin Server` para que funcione con `MySQL` y ahora si vamos a la url
`http://127.0.0.1:9411/` podremos ver sin problemas la `UI` de `Zipkin`.

![22.png](assets/section-10/22.png)

## Probando MySQL Storage en Zipkin Server

Ejecutaremos nuestros microservicios en el siguiente orden.
...
Luego de haber levantado los microservicios podemos hacer alguna petición y ver los resultados en la web de `zipkin`.

![23.png](assets/section-10/23.png)

Ahora, todos los registros mostrados en la web de `Zipkin` se están guardando en la base de datos de `MySQL`.

![24.png](assets/section-10/24.png)
