...
## Consumiendo API Rest con FeignClient desde item-service

En este apartado usaremos `FeignClient` para comunicarnos con `product-service`. Entonces, en la interfaz
`ProductFeignClient` debemos agregar tres métodos más para poder implementar el registrar, actualizar y eliminar de
productos. Estos métodos tienen que ser similar a lo que tenemos en el controlador del `product-service`.
...
En la clase concreta `ItemserviceImple` implementamos los métodos usando la interfaz anterior.
...
Como en el `item-service` tenemos dos implementaciones del `ItemService` y en esta oportunidad necesitamos probar la
implementación concreta `ItemServiceImpl`, necesitamos hacer una modificación en el `ItemController`. En la anotación
`@Qualifier("itemServiceImpl")` tenemos que definir el nombre el bean que queremos que se inyecte, para eso debemos
colocar el nombre de la clase concreta iniciando con minúscula.
...
< Location: /api/v1/products/10
< Content-Type: application/json
< Date: Tue, 15 Apr 2025 17:46:56 GMT
< token-response: 123456
< X-Response-color: green
< set-cookie: color=red
<
...
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Tue, 15 Apr 2025 17:47:33 GMT
< token-response: 123456
< X-Response-color: green
< set-cookie: color=red
<
...
>
< HTTP/1.1 204 No Content
< Date: Tue, 15 Apr 2025 17:48:11 GMT
< token-response: 123456
< X-Response-color: green
< set-cookie: color=red
<
