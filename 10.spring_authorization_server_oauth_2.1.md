...
8. `AuthorizationServerSettings`, es un bean que define las configuraciones del servidor de autorización, y por defecto,
   `Spring Security OAuth2 Authorization Server` expone los endpoints estándar como `/oauth2/authorize`,
   `/oauth2/token`, etc. En mi caso he configurado el `issuer` a `http://127.0.0.1:9000`, lo que sugiere que el servidor
   de autorización estará escuchando en el puerto `9000` de `localhost`. El `issuer` es utilizado por los clientes para
   identificar la autoridad que ha emitido un token y es comúnmente parte de los metadatos de configuración en un
...
- Esto indica que el `gateway-server` usará `JWT` para `validar tokens y proteger rutas`.
...
- Estamos registrando un cliente llamado `my-oauth2-client` que apunta al `authorization-server` para
