...
`Spring Cloud Gateway` relaciona rutas como parte de la infraestructura `HandlerMapping` de `Spring WebFlux`.
`Spring Cloud Gateway` incluye numerosas fábricas de predicados de ruta integradas. Todos estos predicados se
relacionan con diferentes atributos de la solicitud HTTP. Se pueden combinar varias fábricas de predicados de ruta con
sentencias lógicas.

> Un `predicate` (o predicado) es una condición lógica que debe cumplirse para que se aplique una ruta.
> En otras palabras:` si el predicado se cumple → el filtro se ejecuta y se enruta la solicitud`.

Hasta el momento tenemos dos rutas `product-service-route` e `item-service-route`. Cada una de estas rutas tiene
definido un predicado llamado `Path`, donde solo si el path de la solicitud empieza con `/api/v1/products/` o
`/api/v1/items/`, entonces se usa la ruta determinada.
...
Ahora, vamos a agregar un predicado más a la ruta `product-service-route`, de tal manera que cuando se trate de hacer
una redirección a esa ruta, como condición lógica es que en los headers se envíe `X-Request-Id` cuyo valor será
numérico. Para saber si el valor enviado es un valor numérico, usamos la expresión regular `\d+`.
...
Para probar el funcionamiento del predicado `Header` que se agregó, vamos a realizar una petición al microservicio de
productos a través del `gateway-server`. Si nos damos cuenta, el predicado `Path` es correcto, dado que la url de
la petición de abajo coincide con el definido en los `predicates` de la ruta `product-service-route`, pero el predicado
`Header` no coincide, dado que en la petición no hemos enviado ningún header con el esperado en esa ruta por eso es que
la petición falla.
...
<
{
  "timestamp": "2025-04-10T16:36:49.942+00:00",
  "path": "/api/v1/products/1",
  "status": 404,
  "error": "Not Found",
  "requestId": "e967dc44-1",
  "message": "No static resource api/v1/products/1."
}
````

Ahora enviemos en el request el `Header` esperado en los filters de la ruta `product-service-route` para que se cumpla
la condición y podamos ser redireccionados al microservicio `product-service`.

````bash
$ curl -v -H "X-Request-Id: 1234567890" http://localhost:8090/api/v1/products/1 | jq
...
< transfer-encoding: chunked
< Content-Type: application/json
< Date: Thu, 10 Apr 2025 16:42:34 GMT
< token-response: 123456
< set-cookie: color=red
< set-cookie: userCookie=magadiflo
<
