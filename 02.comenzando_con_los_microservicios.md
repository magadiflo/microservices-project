...
## Configurando RestClient para consumir nuestra API products
...
> de tipo `RestClient.Builder` anotado con `@LoadBalanced`. Esto permite que el cliente resuelva nombres de servicios
> lógicos (como `lb://product-service`) usando el sistema de descubrimiento (por ejemplo, `Eureka`).
>
> En cambio, si solo queremos hacer una petición HTTP simple a una URL fija (sin balanceo de carga ni resolución de
> servicios), no es necesario definir un `@Bean` ni usar `@LoadBalanced`; basta con inyectar directamente una instancia
> de `RestClient` en la clase que realizará las solicitudes.
...
A continuación, creamos otra clase de servicio donde inyectamos el `RestClient.Builder` y definimos la url a donde
...
Notar que estamos usando la baseUrl `lb://product-service/api/v1/products`. El valor `product-service` debe ser el
